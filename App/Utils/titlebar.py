...
root.config(bg=LGRAY2, borderwidth=1)
...
title_bar_title = Label(title_bar, text=tk_title, bg=RGRAY,bd=0,fg='white',font=("helvetica", 10),highlightthickness=0, padx=5)
...
def changex_on_hovering(event):
    global close_button
    close_button['bg']='#ae0d1a'
    
def changex_on_click(event):
    global close_button
    close_button['bg']='#f16f7a'
...
title_bar.bind('<Double-Button-1>',maximize_me)

# button effects in the title bar when hovering over buttons
close_button.bind('<Enter>',changex_on_hovering)
close_button.bind('<Button-1>',changex_on_click)
...
resizex_widget = Frame(window,bg="white",cursor="sb_h_double_arrow")
...
resizey_widget = Frame(window,bg="white",cursor='sb_v_double_arrow')
resizey_widget.place(relx=0, rely=1, anchor="sw", relwidth=1, height=5)
...
# resize the window height
resizexy_widget = Frame(window,bg="white",cursor='size_nw_se')
resizexy_widget.place(relx=1, rely=1, anchor="se", width=10, height=10)

root.minsize(width=200, height=200)
def resizexy(event):
    xwin = root.winfo_x()
    xdifference = (event.x_root - xwin) - root.winfo_width()
    ywin = root.winfo_y()
    ydifference = (event.y_root - ywin) - root.winfo_height()

    if root.winfo_width() > 150 : # 150 is the minimum width for the window
        try:
            root.geometry(f"{ root.winfo_width() + xdifference }x{ root.winfo_height() + ydifference}")
        except:
            pass
    else:
...
resizexy_widget.bind("<B1-Motion>",resizexy)
